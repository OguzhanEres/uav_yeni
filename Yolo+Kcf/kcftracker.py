    ...
    def getFeatures(self, image, inithann, scale_adjust=1.0):
        ...
        if inithann:
            ...
            if self.template_size > 1:
                ...
                self._tmpl_sz = (int(padded_w / self._scale), int(padded_h / self._scale))
            else:
                self._tmpl_sz = (int(padded_w), int(padded_h))
                self._scale = 1.0

            if self._hogfeatures:
                self._tmpl_sz = (int(self._tmpl_sz[0] / (2 * self.cell_size)) * 2 * self.cell_size + 2 * self.cell_size,
                                int(self._tmpl_sz[1] / (2 * self.cell_size)) * 2 * self.cell_size + 2 * self.cell_size)
            else:
                self._tmpl_sz = (int(self._tmpl_sz[0] / 2) * 2, int(self._tmpl_sz[1] / 2) * 2)
