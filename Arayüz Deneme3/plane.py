...
class Plane():
    ...
    def land_mission(self):
        """ Adds a lans item to the UAV mission, if it's not defined yet
        
        Input:
            land_pitch       - [deg] pitch angle during land
            heading             - [deg] heading angle during land (default is the current)
        """
        self.clear_mission()
        self.set_ap_mode("AUTO")
        #cmds = plane.vehicle.commands
        land_mission = Command(0,0,0,3, mavutil.mavlink.MAV_CMD_NAV_LAND,0,0,0,0,0,0,self.location_home.lat,self.location_home.lon,self.location_home.alt)
        self.mission.add(land_mission)
                    #cmds.add(wp)
                    #cmds.upload()
        self.vehicle.flush()
        print("Plane is descending")
        cmd_set = False
        if self.location_current.alt < 1:
            landed = True
            self.set_ap_mode("Manual")
            self.disarm()
    ...
    def uav_server(self):
            ...
            while True:
                ...
                while conn_status :
                        send = {"Status" : "OK"} 
                        try:
                            self.conn.send(str(send).encode())
                ...
                while conn_status == False:
                    ...
                    try:
                        self.conn.send(str(reconnected).encode())
                        conn_status = True
                        break
                    except socket.error:
                        conn_status = False
...
class TelemetryCreate:
    def __init__(self) -> None:
        self.iha_bilgiler = {"takim_numarasi": int,
            "IHA_enlem": float,
            "IHA_boylam": float,
            "IHA_irtifa": float,
            "IHA_dikilme": int,
    ...
    def update_telemetry(self):
        self.iha_bilgiler["takim_numarasi"] = takim_no
        ...
        self.iha_bilgiler["IHA_havahiz"] = plane.airspeed
        self.iha_bilgiler["IHA_yerhiz"] = plane.airspeed
        self.iha_bilgiler["IHA_mod"] = plane.get_ap_mode()
        self.iha_bilgiler["IHA_isarmeble"] = plane.vehicle.is_armable
        self.iha_bilgiler["IHA_armed"] = plane.is_armed()
        self.iha_bilgiler["IHA_batarya"] = plane.vehicle.battery.level
        self.iha_bilgiler["IHA_otonom"] = plane.get_ap_mode != "Manual"
        return self.iha_bilgiler
...
def send_telemetry_data():
    ...
    uav_server_object.conn.send(str(ongoing_telemetry_data).encode)
    return True
